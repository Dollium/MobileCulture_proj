...
<head>
  ...
  .update_btn{
    ...
    .update_btn:hover, .update_btn:active {
        color: #49515e;
           ...
           if(!empty($_SESSION['success_registration'])){
            ...
            <table id="user_log" class="user_log" style="overflow-x:auto; max-width:1140px;">
              ...
              while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>" . $row['Year'] . "</td>";
                echo '<td><form action="update.php" method="post" enctype="multipart/form-data" id="form-update">'.
                  		'<input type="hidden" name="id" value="'. $row['userid'] .'"><input type="hidden" name="scl_id" value="'. $row['scl_id'] .'">'.
               		    '<input type="submit" value="&#xf044;" id="update" class="update_btn">'.
                      '</form>'.
                      '</td>';
